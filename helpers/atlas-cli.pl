...
#	delver($cloudname, $version) && exit(0);
#	createver();
#	checkver();
#	update();
#	createprovider();
...
#	uploadbox(get_uploadpath());
    ...
    createver($cloudname,  $version, $version_log)
	&& createprovider($cloudname, $version, $provider)
	&& uploadbox(get_uploadpath($cloudname, $version, $provider), $codename);
}
